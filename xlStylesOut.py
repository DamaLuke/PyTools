...
# 默认格式
title_font = Font(
    name=u"微软雅黑", size=10, bold=True, color="00FFFFFF"
)
title_fill = PatternFill(
    fill_type="solid", start_color="3D5AAE", end_color="3D5AAE"
)
body_font = Font(
    name=u"微软雅黑", size=9, bold=False, color="000000"
)
        ...
        for ws in wb:
            for col in range(ws.min_column, ws.max_column + 1):
                ...
                for i in range(1, headers + 1):
                    title_cell = ws.cell(i, col)
                    title_cell.style = titleStyle
                    for row in range(ws.min_row + 1, ws.max_row + 1):
                        body_cell = ws.cell(row, col)
                        body_cell.style = bodyStyle
